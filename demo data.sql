...
select first_name,last_name from employees --select by own
select distinct first_name from employees  -- select unique
select top 5 * from employees order by first_name
select * from employees where salary>4000  --  sorting
select * from employees where phone_number is null --check not null
select * from employees where phone_number is not null And salary>8000  -- check two columns using logical AND OR NOT
select * from employees where not (salary<=8000 and salary>3000) -- using not get reverse result set

select * from employees where first_name like 's%'  -- filtering the text feild coulmn
select * from employees where first_name like 's____' -- eject know who much character
select * from employees where first_name like 's[au]%' --if we know some middle characters
select * from employees where first_name like 's[^au]%'-- just like not

select * from employees where employee_id in (116,117,120) -- to get random ids

select * from dependents where employee_id in  -- using this filter to another table to this table
(select employee_id from employees where salary<4000)  -- sub query

select * from employees where employee_id between 120 and 190 -- to specify range

select first_name as fn,last_name as ln,email as mail from employees as emp  -- alias
select salary*1.5 as es from employees as emp -- custom alias for our operation

select department_id,count(employee_id) as empCount from employees group by department_id --grouping rows

select department_id,count(employee_id)asempCount,Min(salary),MAX(salary) -- aggreagate functions
from employees group by department_id
select department_id,count(employee_id)asempCount,Min(salary),MAX(salary) -- aggreagate functions
from employees group by department_id having count(employee_id)>2  -- to make condition in group by just like where

select first_name,last_name from employees union select first_name,last_name from dependents -- two combine two tables with same data type

select * from employees as e join jobs as j on e.job_id = j.job_id --joins by default inner join
select e.employee_id,j.job_id,job_title from employees as e join jobs as j on e.job_id = j.job_id -- to get specfic colums

select employee_id,isnull(phone_number,'78234872635') from employees -- return fixed value that is null

select employee_id,  -- if else
case
    when salary<5000 then 'below avg'
